...
namespace PnPConverter
{
  ...
  public class PnPReel
  {
    ...
    public static string SerializeReel(PnPReel reel)
    {
      ...
      sReelData.Append(reel.PartNameOrValue);
      sReelData.Append('|');
      ...
      sReelData.Append(reel.PartSize.X.ToString());
      sReelData.Append('|');
 
      sReelData.Append(reel.PartSize.Y.ToString());
      sReelData.Append('|');
      ...
      sReelData.Append(reel.Offset.X.ToString());
      sReelData.Append('|');

      sReelData.Append(reel.Offset.Y.ToString());
      sReelData.Append('|');
    ...
    public static PnPReel DeSerializeReel(string reelString, out bool errors)
    {
      ...
      /* There should be 20 properties per Reel:
       * - GUID
       * - PartNumber or Value
       * - Manufacturer
       * - Supplier
      ...
      */
      if (sProperties.Length == 20)
      {
        ...
        string sSizeX = sProperties[13];
        string sSizeY = sProperties[14];
        float fSizeX, fSizeY;
        float.TryParse(sSizeX, out fSizeX);
        float.TryParse(sSizeY, out fSizeY);
        FSize xPartSize = new FSize(fSizeX, fSizeY);
        xNewReel.PartSize = xPartSize;

        PnPMachineNozzle xNozzle = PnPMachineNozzle.Undefined;
        switch (sProperties[15])
        {
          case "XS":
        ...
        string sSpeed = sProperties[16];
        int iSpeed;
        int.TryParse(sSpeed, out iSpeed);
        xNewReel.Speed = iSpeed;

        string sOffsetX = sProperties[17], sOffsetY = sProperties[18];
        ...
        xNewReel.Offset = new FPoint(fOffsetX, fOffsetY);

        string sRotation = sProperties[19];
        int iRotation = 0;
